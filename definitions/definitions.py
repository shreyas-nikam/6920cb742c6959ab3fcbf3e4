...
def apply_scenario_shock_to_features(features_df, shock_vector_dict):
    ...
    for feature, shock_multiplier in shock_vector_dict.items():
        if feature in stressed_features_df.columns:
            stressed_features_df[feature] = stressed_features_df[feature] * shock_multiplier
...
import pickle
            ...
            def load_financial_model(model_path):
                ...
                try:
                    ...
                    raise OSError(f"Failed to load model from '{model_path}': Corrupted file or invalid format. Original error: {e}") from e
                    ...
                    if isinstance(e, OSError):
                        raise
                    raise OSError(f"An unexpected error occurred while loading model from '{model_path}': {e}") from e
...
def get_baseline_model_predictions(model, original_features_df):
    ...
    if not hasattr(model, 'predict'):
        raise AttributeError("The provided model object must have a 'predict' method.")
...
def calculate_individual_prediction_impact(baseline_predictions, stressed_predictions):
    ...
    if not isinstance(baseline_predictions, (pd.Series, np.ndarray)) or \
       not isinstance(stressed_predictions, (pd.Series, np.ndarray)):
        raise TypeError("Input predictions must be pandas Series or numpy arrays.")

    # Validate lengths
    if len(baseline_predictions) != len(stressed_predictions):
        raise ValueError("Baseline and stressed predictions must have the same length.")
...
def aggregate_portfolio_impact_metrics(individual_prediction_impacts, portfolio_weights, metric_type):
    ...
    if portfolio_weights is not None:
        ...
        if weights.size != impacts.size:
            raise ValueError("portfolio_weights must have the same size as individual_prediction_impacts.")
    ...
    if metric_type == "mean":
        ...
        try:
            ...
            raise ValueError(f"Invalid percentile metric type: {metric_type}. Expected format 'percentile_XX'.") from e
    else:
...
def identify_risk_grade_migrations(baseline_risk_grades, stressed_risk_grades):
    ...
    if not isinstance(baseline_risk_grades, pd.Series) or not isinstance(stressed_risk_grades, pd.Series):
        raise TypeError("Both baseline_risk_grades and stressed_risk_grades must be pandas Series.")

    # Find common obligors to ensure a fair comparison across the same set of entities.
    common_obligors = baseline_risk_grades.index.intersection(stressed_risk_grades.index)
    ...
    for alpha in alphas:
        ...
        for feature, shock_multiplier in base_shock_vector_dict.items():
            ...
            if feature in current_stressed_df.columns:
                ...
                current_stressed_df[feature] = current_stressed_df[feature] * (1 + alpha * shock_multiplier)
...
def plot_model_response_trajectory(alpha_values, model_output_trajectories_df, output_metric_name):
    ...
    if output_metric_name not in model_output_trajectories_df.columns:
        raise KeyError(f"Output metric '{output_metric_name}' not found in DataFrame columns.")

    if len(alpha_values) != len(model_output_trajectories_df):
        raise ValueError("Length of 'alpha_values' must match the number of rows in 'model_output_trajectories_df'.")
    ...
    # Plot the trajectory
    ax.plot(alpha_values, y_data, marker='o', linestyle='-', label=output_metric_name)
...
def generate_scenario_comparison_report_table(baseline_metrics_dict, stressed_metrics_dict):
    ...
    all_metrics_keys = sorted(list(set(baseline_metrics_dict.keys()) | set(stressed_metrics_dict.keys())))
...
def generate_segment_impact_heatmap(segmentation_data_df, impact_data_series, segment_column):
    ...
    if segment_column not in segmentation_data_df.columns:
        raise KeyError(f"Segment column '{segment_column}' not found in segmentation_data_df.")
    ...
    if impact_col_name in segmentation_data_df.columns:
        impact_col_name = '___segment_impact_data_temp___' # Fallback for extremely rare collision
    ...
    if processed_df.empty:
        fig, ax = plt.subplots(figsize=(8, 1)) # Create a small figure for placeholder message
        ax.set_title(f"Segment Impact by '{segment_column}' (No Valid Data)", fontsize=14)
        ax.set_xticks([])
        ax.set_yticks([])
    ...
    segment_impact = processed_df.groupby(segment_column)[impact_col_name].mean()
    ...
    fig_height = max(1.5, num_segments * 0.6) # Minimum height, plus 0.6 inches per segment
    ...
    sns.heatmap(
        ...
        cmap="coolwarm",         # Diverging colormap, suitable for showing positive/negative impacts
        ...
        yticklabels=True         # Ensure y-axis labels (segment names) are shown
    )

    ax.set_title(f"Average Impact by {segment_column}", fontsize=16)
    ax.set_xlabel("") # Remove the x-axis label as it's implicitly 'Average Impact' from the column name
    ax.set_ylabel(segment_column, fontsize=14)
    plt.yticks(rotation=0, fontsize=12) # Keep segment labels horizontal for better readability
    plt.xticks([]) # Hide the x-tick label which would be 'Average Impact' for the single column
...
def plot_delta_prediction_distribution(delta_predictions_series, threshold=None):
    ...
    if not isinstance(delta_predictions_series, (pd.Series, np.ndarray)):
        raise TypeError("delta_predictions_series must be a pandas.Series or numpy.ndarray.")
    ...
    if threshold is not None:
        effective_threshold = abs(threshold) # Use the absolute value for the threshold magnitude.
        ...
        epsilon = 1e-9 # A small offset to ensure shading starts just after the vertical lines.
        ax.axvspan(effective_threshold + epsilon, x_max, color='red', alpha=0.1)
        ax.axvspan(x_min, -effective_threshold - epsilon, color='red', alpha=0.1,
                   label='Exceeds Threshold Region') # Label for the shaded area in the legend.
    ...
    if ax.get_legend_handles_labels()[0]: # Check if any handles (plot elements with labels) exist.
        ax.legend()
    
    plt.tight_layout() # Adjust plot layout to prevent labels or titles from overlapping.
