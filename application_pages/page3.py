...
def run_page3():
    ...
    *   Counts or percentages of obligors whose risk grade upgrades/downgrades under a specific scenario $s$.
    ...
    agg_income_scale = st.slider(
        "Income Scale Factor (for Aggregation)", 0.5, 1.5, 0.9, 0.05)
    agg_dti_scale = st.slider(
        "Debt-to-Income Ratio Scale Factor (for Aggregation)", 0.5, 1.5, 1.2, 0.05)
    agg_util_scale = st.slider(
        "Utilization Rate Scale Factor (for Aggregation)", 0.5, 1.5, 1.1, 0.05)

    delta_s_agg = np.array(
        [agg_income_scale - 1, agg_dti_scale - 1, agg_util_scale - 1])
    ...
    X_stressed_agg['debt_to_income_ratio'] = X_stressed_agg['debt_to_income_ratio'].clip(
        lower=0, upper=1)
    X_stressed_agg['utilization_rate'] = X_stressed_agg['utilization_rate'].clip(
        lower=0, upper=1)
    ...
    df_portfolio['Baseline_Risk_Grade'] = df_portfolio['Baseline_PD'].apply(
        get_risk_grade)
    df_portfolio['Stressed_Risk_Grade'] = df_portfolio['Stressed_PD'].apply(
        get_risk_grade)

    st.write(
        f"Mean Baseline PD (Portfolio): `{df_portfolio['Baseline_PD'].mean():.4f}`")
    st.write(
        f"Mean Stressed PD (Portfolio): `{df_portfolio['Stressed_PD'].mean():.4f}`")
    st.write(
        f"Mean Change in PD (Portfolio): `{df_portfolio['PD_Change'].mean():.4f}`")

    st.subheader("Risk Grade Migration Under Stress")
    grade_migration = pd.crosstab(
        df_portfolio['Baseline_Risk_Grade'], df_portfolio['Stressed_Risk_Grade'], normalize='index').mul(100).round(2)
    ...
    sens_income_shock = st.number_input(
        "Income Shock (e.g., -0.1 for 10% drop)", value=-0.1, step=0.01, format="%.2f")
    sens_dti_shock = st.number_input(
        "DTI Shock (e.g., 0.2 for 20% increase)", value=0.2, step=0.01, format="%.2f")
    sens_util_shock = st.number_input(
        "Utilization Shock (e.g., 0.15 for 15% increase)", value=0.15, step=0.01, format="%.2f")

    # Define the base delta_s for the sensitivity analysis
    base_delta_s = np.array(
        [sens_income_shock, sens_dti_shock, sens_util_shock])
    ...
    for alpha in alpha_values:
        ...
        X_alpha_stressed = pd.DataFrame(
            X_alpha_stressed_values, columns=X.columns)

        # Ensure non-negative/within bounds
        X_alpha_stressed['income'] = X_alpha_stressed['income'].clip(lower=0)
        X_alpha_stressed['debt_to_income_ratio'] = X_alpha_stressed['debt_to_income_ratio'].clip(
            lower=0, upper=1)
        X_alpha_stressed['utilization_rate'] = X_alpha_stressed['utilization_rate'].clip(
            lower=0, upper=1)

        # Apply scaling
        X_alpha_stressed_scaled = scaler.transform(X_alpha_stressed)

        stressed_predictions_alpha = model.predict_proba(
            X_alpha_stressed_scaled)[:, 1]
        mean_stressed_pds.append(np.mean(stressed_predictions_alpha))

    fig_sens = go.Figure()
    fig_sens.add_trace(go.Scatter(x=alpha_values, y=mean_stressed_pds,
                       mode='lines+markers', name='Mean Stressed PD'))
