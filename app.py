...
st.title("QuLab")
st.divider()
st.markdown("""In this lab, we explore scenario-based robustness testing of financial Machine Learning (ML) models. The core idea is to apply structured stress transformations ($T_s(x)$) on input features to observe how a model's predictions ($f_\theta(x^{(s)})$) change, rather than retraining the model for every scenario.
